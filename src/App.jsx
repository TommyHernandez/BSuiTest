...
function App() {
  ...
  const handlesources = () => (cat, lang) => {
    restApi.getSources(cat, lang).then((sdata) => {
      setSources(sdata);
    });
  };
  ...
  return (
    <div className="app-bs-news">
      ...
      <div className="side">
        <div
          className={
            toggleFilter ? 'side-container' : 'side-container collapsed'
          }
        >
          <FilterComponent handleGetSources={handlesources()} />
          <SourceContainer
            sources={sources}
            handleNews={handleNewsBySource()}
          />
        </div>
      </div>
      <div className="content-container">
        ...
        <div className="news-list">
          {!showLoader &&
            (news.length ? (
              news.map((n, i) => <ArticleCard {...n} key={`cr-${i}`} />)
            ) : (
              <h3 className="warning">Not Results found</h3>
            ))}
        </div>
      </div>
      <footer>
